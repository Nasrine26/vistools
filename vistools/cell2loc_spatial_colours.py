...
def plot_spatial_general(
    ...
    if colordict_celltype_rgb:

        cmaps = list()
        for cell2plot in value_df.columns:
            celltype1 = create_colormap(colordict_celltype_rgb[cell2plot][0],
                                        colordict_celltype_rgb[cell2plot][1],
                                        colordict_celltype_rgb[cell2plot][2]
                                        )

            cmaps.append(celltype1)
...
def plot_spatial(adata, color, img_key="hires", show_img=True, colordict_celltype_rgb=None, **kwargs):
    ...
    if "spatial" in adata.uns.keys():
        kwargs["coords"] = (
                adata.obsm["spatial"] * list(adata.uns["spatial"].values())[0]["scalefactors"][
            f"tissue_{img_key}_scalef"]
        )
    else:
        kwargs["coords"] = adata.obsm["spatial"]

    fig = plot_spatial_general(value_df=adata.obs[color], colordict_celltype_rgb=colordict_celltype_rgb,
                               **kwargs)  # cell abundance values
